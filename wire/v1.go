...
func (m *Message) WriteTo(w io.Writer) (int64, error) {
	...
	log.Trace().Msg("actually writing")
...
func ReadFrom(r io.Reader) (*Message, error) {
	...
	for rep[0] != repMarker || rep[1] != repMagic || rep[2] != repMagic {
		log.Trace().Msg("detected previous message, skipping")
		n, err = r.Read(rep[:])
		if err != nil {
			return nil, err
		}
	...
	log.Trace().Msg("actual reading started")
	...
	log.Trace().Msg("actual reading finished")
